...
def project_position(object_position, object_height):
    h = CAMERA_HEIGHT - BALISE_HEIGHT
    vector_coordinates = object_position - camera_position
#    Convert to polar coordinates
    r = np.linalg.norm(vector_coordinates)
    theta =np.arctan(vector_coordinates[1]/vector_coordinates[0])
    proj_r = r * h /  CAMERA_HEIGHT
    proj_vector = np.array([proj_r * np.cos(theta), proj_r * np.sin(theta)])
    proj_position = proj_vector + camera_position
